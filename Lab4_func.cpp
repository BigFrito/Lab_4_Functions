...
*/
int isGreaterThan(double x, int y){
    return x > y;
}
...
string select(string func_type){
    if (func_type == "sumOfThree"){
        out = printf("The sum is ");
    }else if (func_type == "division"){
        out = printf("The quotient is ");
    }else if (func_type == "isGreaterThan"){
        out = printf("The first value being greater than the second value is ");
    }else{
        out = printf("Invalid function type\n");
    }
    return out;
}
 ...
 * 
 * @tparam T : (int, double, string) output of sum, divison, or isGreater Than
 * @param func_type : (string)
 * @param output
*/
template <typename T>
